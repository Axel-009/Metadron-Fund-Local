...
export default function MacroDashboard() {
  ...
  // Historical time series from FRED (no more Math.random)
  const { data: spreadHistData } = useEngineQuery<{ data: ChartPoint[] }>("/macro/spread-history", { refetchInterval: 300000 });
  const { data: vixHistData } = useEngineQuery<{ data: ChartPoint[] }>("/macro/vix-history", { refetchInterval: 300000 });
  const { data: dxyHistData } = useEngineQuery<{ data: ChartPoint[] }>("/macro/dxy-history", { refetchInterval: 300000 });

  // ─── Derived data ─────────────────────────────────────────

  // News from API
  const macroNews: NewsItem[] = useMemo(() => {
    if (!newsData?.news?.length) return [];
    return newsData.news.slice(0, 10).map((n) => ({
      ts: n.time || n.date || n.published || "",
  ...
  const g10Countries: G10Country[] = useMemo(() => {
  ...
  const econCalendar: CalendarEvent[] = useMemo(() => {
  ...
  // Regime from API snapshot
  const apiRegime = macroSnap?.regime?.toUpperCase() || "UNKNOWN";
  const regime = {
    label: apiRegime,
    confidence: macroSnap ? Math.round((macroSnap.gmtf_score || 0) * 100) : 0,
    prev: "—",
  };

  // Liquidity metrics from velocity API
  const liqMetrics: LiquidityMetric[] = useMemo(() => {
    if (!velocityData) return [];
    return [
      { label: "Liquidity Score", value: velocityData.liquidity_score != null ? `Score: ${(velocityData.liquidity_score).toFixed(1)}` : "—", chg: "", dir: "up" },
  ...
  // Chart data from FRED historical endpoints
  const spreadData = spreadHistData?.data || [];
  const vixData = vixHistData?.data || [];
  const dxyData = dxyHistData?.data || [];
    ...
    <div className="h-full flex flex-col gap-1 p-1 overflow-hidden">
      ...
      <div className="flex-shrink-0 terminal-panel flex items-center justify-between px-3 py-1" style={{ border: `1px solid ${r.border}`, backgroundColor: r.bg }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[18px] font-mono font-bold tracking-widest" style={{ color: r.text }}>
            {macroSnap ? regime.label : "LOADING..."}
          </span>
          {macroSnap && (
            <>
              ...
              <span className="text-[9px] text-terminal-text-faint">GMTF: {macroSnap.gmtf_score?.toFixed(3) || "—"}</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-3">
      ...
      <DashboardPanel title="G10 ECONOMIC INDICATORS" noPadding>
        <div className="overflow-auto h-full">
          {g10Countries.length === 0 ? (
            <div className="flex items-center justify-center h-full text-terminal-text-muted text-[10px] font-mono">Loading G10 data from FRED...</div>
          ) : (
          ...
          {/* Right: 3 Charts from FRED historical data */}
          <div className="flex flex-col gap-1 h-full overflow-auto">
            <DashboardPanel title="2s10s YIELD SPREAD (30D)" className="flex-1">
              {spreadData.length === 0 ? (
                <div className="flex items-center justify-center h-full text-terminal-text-muted text-[10px] font-mono">Loading FRED spread data...</div>
              ) : (
            ...
            <DashboardPanel title="VIX INDEX (30D)" className="flex-1">
              {vixData.length === 0 ? (
                <div className="flex items-center justify-center h-full text-terminal-text-muted text-[10px] font-mono">Loading FRED VIX data...</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
            ...
            <DashboardPanel title="DXY DOLLAR INDEX (30D)" className="flex-1">
              {dxyData.length === 0 ? (
                <div className="flex items-center justify-center h-full text-terminal-text-muted text-[10px] font-mono">Loading FRED DXY data...</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="flex-shrink-0 h-40">
        <ResizableDashboard defaultSizes={[60, 40]} minSizes={[30, 25]}>
          <DashboardPanel title="MACRO NEWS FEED" noPadding>
            <div className="overflow-auto h-full">
              {macroNews.length === 0 ? (
                <div className="flex items-center justify-center h-full text-terminal-text-muted text-[10px] font-mono">Loading news feed...</div>
              ) : (
                macroNews.map((n, i) => (
                  <div key={i} className={`px-2 py-1 border-b border-terminal-border flex items-start gap-2 hover:bg-[#161b22] ${i % 2 === 0 ? "" : "bg-[#0d1117]"}`}>
                    ...
                    <span className="text-[8px] font-mono px-1 rounded flex-shrink-0" style={{ backgroundColor: (SENTIMENT_COLORS[n.sentiment] || SENTIMENT_COLORS.neutral) + "22", color: SENTIMENT_COLORS[n.sentiment] || SENTIMENT_COLORS.neutral }}>
                      {n.sentiment}
                    </span>
                  </div>
                ))
          ...
          <DashboardPanel title="MONEY VELOCITY & LIQUIDITY">
            <div className="grid grid-cols-1 gap-2">
              {liqMetrics.length === 0 ? (
                <div className="flex items-center justify-center text-terminal-text-muted text-[10px] font-mono">Loading velocity data...</div>
              ) : (
                liqMetrics.map(m => (
                  <div key={m.label} className="flex items-center justify-between border-b border-terminal-border pb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {m.chg && <span className={`text-[9px] font-mono ${m.dir === "up" ? "text-terminal-positive" : "text-terminal-negative"}`}>{m.chg}</span>}
                    </div>
                  </div>
                ))
              )}
