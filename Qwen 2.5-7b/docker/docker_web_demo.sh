...
while [[ "$1" != "" ]]; do
    case $1 in
        -i | --image-name ) shift; IMAGE_NAME=$1 ;;
        -c | --checkpoint ) shift; QWEN_CHECKPOINT_PATH=$1 ;;
        -n | --container-name ) shift; CONTAINER_NAME=$1 ;;
        --port ) shift; PORT=$1 ;;
        --flash-attn2 ) FLASH_ATTN=1 ;;
        -h | --help ) usage; exit 0 ;;
...
sudo docker pull ${IMAGE_NAME} || { echo "Pulling image ${IMAGE_NAME} failed, exit."; exit 1; }

WEB_DEMO_ARGS="--server-port 8901 --server-name 0.0.0.0 -c /data/shared/Qwen/Qwen2.5-Omni-7B"
if [ ${FLASH_ATTN} -eq 1 ]; then WEB_DEMO_ARGS+=" --flash-attn2"; fi
